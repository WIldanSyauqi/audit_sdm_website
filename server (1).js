...
// JWT_SECRET wajib di-set lewat env var saat production. Kita TIDAK pakai
// process.exit() di sini — di lingkungan serverless (Vercel), process.exit()
// mematikan seluruh function process dan bikin SEMUA request (termasuk file
// statis) gagal dengan 500 FUNCTION_INVOCATION_FAILED. Sebagai gantinya kita
// set flag, dan endpoint yang butuh JWT_SECRET (login/authMiddleware) yang
// akan menolak request dengan pesan error yang jelas.
const JWT_SECRET_MISSING = process.env.NODE_ENV === 'production' && !process.env.JWT_SECRET;
if (JWT_SECRET_MISSING) {
  console.error('WARNING: JWT_SECRET belum di-set di environment variables production. Login/auth akan dinonaktifkan sampai env var ini di-set di Vercel > Settings > Environment Variables.');
}
...
// Kalau ALLOWED_ORIGINS di-set (pisahkan dengan koma), CORS dibatasi ke
// origin itu saja. Kalau tidak di-set, default masih allow-all supaya
// tidak mendadak memutus akses sebelum kamu sempat konfigurasi env var-nya.
...
// Rate limiter sederhana khusus endpoint login (in-memory, per instance).
// Cukup untuk mencegah brute-force kasar; kalau butuh proteksi lebih kuat
// di banyak instance sekaligus, ganti dengan express-rate-limit + store
// eksternal (mis. Redis / Upstash).
...
// Mengubah placeholder gaya SQLite ('?') jadi gaya Postgres ('$1, $2, ...')
// secara berurutan. Semua query di file ini ditulis pakai '?', sedangkan
// driver 'pg' hanya mengerti '$1, $2, ...' — tanpa konversi ini SEMUA query
// akan gagal saat DB_CLIENT=postgres.
...
async function connectDatabase() {
  if (DB_CLIENT === 'postgres') {
    if (!process.env.DATABASE_URL) {
      throw new Error('DATABASE_URL belum di-set (wajib untuk DB_CLIENT=postgres).');
    }

    // Pakai Pool, bukan Client tunggal. Di lingkungan serverless (Vercel),
    // satu koneksi Client bisa "stale"/putus di antara invocation function.
    // Pool otomatis membuka/menutup/reuse koneksi per query, dan cocok
    // dipakai bareng connection pooler Neon.
    pgClient = new Pool({
    ...
    pgClient.on('error', (err) => {
      // Jangan biarkan koneksi idle yang error mematikan seluruh function.
...
// Catatan: koneksi TIDAK lagi dibuka langsung saat module di-load.
// Di Vercel serverless, ini dipicu lewat middleware initializeDatabase()
// di request pertama tiap cold start (lihat app.use(...) di bawah).
// Untuk npm start biasa (Railway/Render/local), startServer() di bagian
// paling bawah file ini yang memicunya.

function run(sql, params = []) {
  if (DB_CLIENT === 'postgres') {
    const pgSql = toPostgresPlaceholders(sql);
    return pgClient.query(pgSql, params).then((result) => ({
...
function all(sql, params = []) {
  if (DB_CLIENT === 'postgres') {
    const pgSql = toPostgresPlaceholders(sql);
    return pgClient.query(pgSql, params).then((result) => result.rows);
  }
...
function get(sql, params = []) {
  if (DB_CLIENT === 'postgres') {
    const pgSql = toPostgresPlaceholders(sql);
    return pgClient.query(pgSql, params).then((result) => result.rows[0] || null);
  }
...
async function initializeDatabase() {
  ...
  if (DB_CLIENT === 'postgres' && !pgClient) {
    await connectDatabase();
  } else if (DB_CLIENT === 'sqlite' && !db) {
    await connectDatabase();
  }
...
function authMiddleware(req, res, next) {
  if (JWT_SECRET_MISSING) {
    return res.status(500).json({ error: 'Server misconfigured: JWT_SECRET belum di-set.' });
  }
...
app.post('/api/auth/login', loginRateLimiter, async (req, res) => {
  if (JWT_SECRET_MISSING) {
    return res.status(500).json({ error: 'Server misconfigured: JWT_SECRET belum di-set. Hubungi admin.' });
  }
